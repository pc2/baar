...
llvm::raw_ostream &CWriter::printType(llvm::raw_ostream &Out, llvm::Type *Ty,
  ...
  case llvm::Type::PointerTyID: {
    ...
    /*if (PTy->getElementType()->isArrayTy() || // TODO recheck!
  ...
  case llvm::Type::ArrayTyID: {
    ...
    // char *
    if (ATy->getElementType() == llvm::Type::getInt8Ty(llvm::getGlobalContext())) {
...
bool CWriter::doInitialization(llvm::Module &M) {
  ...
  /*if (...) { // TODO make sure fast math is desired
    ...
    for (llvm::BasicBlock::iterator iter = bb.begin(), e = bb.end(); iter != e; ++iter) {
        ...
        if (smearFuncName != NULL) {
            ...
            llvm::Instruction *smearCall =
                llvm::CallInst::Create(smearFunc, argArray, smearValue->getName() + "_smear",
...
AndCmpCleanupPass::runOnBasicBlock(llvm::BasicBlock &bb) {
 ...
 restart:
    for (llvm::BasicBlock::iterator iter = bb.begin(), e = bb.end(); iter != e; ++iter) {
        ...
        // int32s vectors.
        if (bop->getType() != llvm::VectorType::get(llvm::Type::getInt1Ty(llvm::getGlobalContext()), 8)) // masktype generic 8
            continue;
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            if (opCmp->getOperand(0)->getType() == llvm::VectorType::get(llvm::Type::getInt1Ty(llvm::getGlobalContext()), 8)) // masktype generic 8
                break;
            ...
            if (andCmpFunc == NULL) {
                ...
                llvm::Constant *acf =
                    module->getOrInsertFunction(funcName, llvm::VectorType::get(llvm::Type::getInt1Ty(llvm::getGlobalContext()), 8),
                                                   cmpOpType, cmpOpType,
                                                   llvm::VectorType::get(llvm::Type::getInt1Ty(llvm::getGlobalContext()), 8), NULL); // masktype generic 8
...
public:
    MaskOpsCleanupPass(llvm::Module *m)
        : BasicBlockPass(ID) {
        llvm::Type *mt = llvm::VectorType::get(llvm::Type::getInt1Ty(llvm::getGlobalContext()), 8); // masktype generic 8
